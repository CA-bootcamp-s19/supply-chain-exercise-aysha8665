...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier forSale(uint256 sku){require(items[sku].state == State.ForSale && items[sku].price >= 0); _;}
  modifier sold(uint256 sku){require(items[sku].state == State.Sold && items[sku].buyer != address(0)); _;}
  modifier shipped(uint256 sku){require(items[sku].state == State.Shipped && items[sku].buyer != address(0)); _;}
  modifier received(uint256 sku){require(items[sku].state == State.Received && items[sku].buyer != address(0)); _;}
